...
export const tips = [
  ...
  },
  {
    ...
    body: [
      ...
      { type: 'paragraph', text: '① 먼저 폴링레이트가 뭔지부터 짚고 가겠습니다. 폴링레이트란 마우스가 1초에 몇 번이나 "지금 위치가 여기예요"라고 PC에 보고하는지를 나타내는 수치입니다. 1000Hz면 1초에 1000번(약 1밀리초 간격), 8000Hz면 1초에 8000번(약 0.125밀리초 간격) 보고합니다. 자주 보고할수록 PC가 마우스 위치를 더 촘촘하게 알 수 있어서, 화면에서 커서나 조준점이 더 부드럽고 즉각적으로 움직이는 느낌을 줄 수 있습니다.' },
      { type: 'callout', text: '한 가지 짚고 넘어갈 부분 — 폴링레이트는 마우스 센서가 움직임을 얼마나 세밀하게 감지하는지(흔히 DPI로 표현하는 "정밀도")와는 다른 개념입니다. 정확히는 감지된 정보를 얼마나 자주 PC로 전달하느냐의 문제라 "정밀도"보다는 "반응성·부드러움"에 더 가깝습니다. 다만 결과적으로 체감 조준감에 영향을 주는 건 맞기 때문에, 편의상 "정밀도가 좋아진다"고 표현해도 큰 무리는 없습니다.' },
      { type: 'paragraph', text: '그런데 숫자로 보면 그 차이가 생각보다 크지 않습니다. 1000Hz와 8000Hz의 보고 간격 차이는 계산상 최대 0.875밀리초(1ms − 0.125ms)에 불과합니다. 사람이 자극에 반응하는 데 보통 150~250밀리초가 걸리는 것과 비교하면, 1밀리초도 안 되는 차이를 실전에서 체감하기는 쉽지 않습니다. 다만 360Hz 이상의 초고주사율 모니터를 쓰면서 빠른 에임을 요구하는 FPS 게임을 하신다면, 이 미세한 차이가 그나마 의미를 가지는 편입니다.' },
      ...
      { type: 'paragraph', text: '③ 두 번째 변수는 지금 플레이하는 게임이 높은 폴링레이트를 얼마나 잘 소화하느냐입니다. 여기서도 한 가지 바로잡을 부분이 있는데, 게임이 높은 폴링레이트를 못 받아준다고 해서 단순히 "효과가 없는 정도"로 끝나는 게 아니라, 오히려 마우스를 빠르게 움직일 때 순간적으로 프레임이 뚝 떨어지거나 화면이 끊기는 역효과로 나타나는 경우가 실제로 보고되고 있습니다. 특히 언리얼 엔진(Unreal Engine) 기반의 최신 게임들에서 이런 사례가 유독 자주 보고되는데, 이 엔진이 애초에 초당 수천 번씩 쏟아지는 마우스 입력을 처리하도록 설계되지 않았기 때문이라는 분석이 많습니다.' },
      ...
      { type: 'callout', text: '결론적으로 폴링레이트는 무조건 높다고 좋은 게 아닙니다. 1000Hz는 2016년부터 게이밍 마우스 업계의 표준으로 자리 잡아 온 만큼 대부분의 상황에서 안정적인 기본값이고, 4000~8000Hz는 CPU 사용률에 여유가 있고 플레이하는 게임이 실제로 잘 받아주는 경우에만 의미가 있습니다. 무작정 최고 수치로 맞추기보다는, 평소 즐기는 게임에서 직접 낮춰가며 프레임이 더 안정적인 지점을 찾아보시는 걸 추천합니다.' },
      ...
      { type: 'table',
        ...
        rows: [
          ...
          ['오버워치 2', '블리자드 자체 엔진'],
          ['도타 2', 'Source 2 엔진'],
          ['에이펙스 레전드', 'Source 엔진(개조) — 타이탄폴 계열'],
          ['레인보우 식스 시즈', 'AnvilNext 2.0(유비소프트 자체 엔진)'],
          ['서든어택', '오래되고 가벼운 개조 엔진'],
        ] },
      ...
      { type: 'table',
        headers: ['게임', '엔진 / 참고'],
        rows: [
          ['배틀그라운드(PUBG)', 'Unreal Engine 4 — 국내 커뮤니티에도 다수 보고'],
          ['포트나이트', 'Unreal Engine 5'],
          ['마블 라이벌스', 'Unreal Engine 5'],
          ['더 퍼스트 디센던트', 'Unreal Engine 5'],
          ['발로란트', '2025년 Unreal Engine 5로 전환 — 아직 폭넓은 이슈 보고는 적으나 주의가 필요'],
        ] },
